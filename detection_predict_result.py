        ...
        if form=='yolo':
            ...
            if isfile(anno):
                ...
                # elif isinstance(file_list, list):
                    ...
                    imgsize = img.size
                    gt_sub['img_W'] = imgsize[0]
                    gt_sub['img_H'] = imgsize[1]
...
def run(pred_file, subset=None):
    ...
    arg = { 'IOU_thresh' : 0.5,
            ...
            'root': os.path.join(os.path.split(pred_file)[0],'data'),
            'form': 'yolo'}
    arg['namesfile'] = f"{arg['root']}/obj.names"

    process_gt(str(Path(pred_file).parent/f'data/{subset}.txt'),subset,**arg)
...
if __name__ == "__main__":
    ...
    for lv in ['lv1','lv2']:
        for subset in ['train','test','valid']:
            pred_file = os.path.expanduser(f'~/Desktop/무제 폴더/{lv}/{subset}_pred_yolov4_best_210609-1200.csv')
            run(pred_file,subset)
